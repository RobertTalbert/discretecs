...
- **Giving an answer of $\binom{20}{5}$ for part 2:** This would represent a situation where we have 20 cards in a stack and we are going to pull out 5 of them, and count the number of ways this can happen. But this assumes that the cards are distinguishable from each other, which is not the case. They are identical, so all selections of 5 cards from the stack will be the same. At any rate, choosing 5 cards from a stack of 20 is not what we are actually doing here, we are giving out all 20 cards to 5 people. 
