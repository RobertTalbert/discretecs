...
- Alice takes her plaintext message --- which for this AEP we will assume is just a string of capital letters without punctuation or other symbols --- removes all spaces, and converts each letter to numbers $00$ through $25$ according to its position in the alphabet. This produces a long string of decimal integers; Alice then breaks that string up into equal-sized blocks, each of which is an integer less than $n$ (from Bob's public key). For example, if Bob's value of $n$ is 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMzMxNTczNTQ2LDQ4MTI5Mzg0OV19
-->