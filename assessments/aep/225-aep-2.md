...
- ...Sharing the key for encryption and decryption was problematic? For example, what I believed that "Eve" might be eavesdropping on the channel I am using to send you the key? I could encrypt the key, but that doesn't solve the problem because how will I get you the second key that encrypts the first one, in a way that convinces me Eve hasn't stolen it or tampered with it? 
- ...I wanted to send you a message but also wanted to remain anonymous --- like a whistleblower who wants to send an encrypted message to a reported about 



But what if I had reason to believe that someone might attack me *en route* to you and steal the key? Or, what if you wanted access to my office but also wanted to remain anonymous? Traditional cryptosystems fail on both counts: They require that the key be communicated over an open channel, and they require Alice and Bob to mutually disclose their identities to each other. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyMDk3NDk5OTcsNDgxMjkzODQ5XX0=
-->