...
1. Python lists can have duplicate elements, while mathematical sets can't.



there is a slight problem with this, because lists can have duplicate elements but sets cannot. For example the list `[10, 10, 20]` is a valid Python list, but $\{10, 10, 20\}$ is not a valid set. (There's also the issue that the ordering of elements in a list matters, but ordering does not matter in a set; however we won't take this issue into account here.) 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTg4MDc5MTA3N119
-->