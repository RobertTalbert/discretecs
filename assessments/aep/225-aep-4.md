...
2. Python lists are ordered, while mathematical sets aren't. For example, the two lists `[3,4,5]` and `[4,3,5]` are considered different, but the two sets $\{3,4,5\}$ and $\{



there is a slight problem with this, because lists can have duplicate elements but sets cannot.  (There's also the issue that the ordering of elements in a list matters, but ordering does not matter in a set; however we won't take this issue into account here.) 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMzc5MDQ5NTQ5XX0=
-->