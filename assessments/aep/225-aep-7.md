...
>dk_constant([1,1,3,5,8,13])
'This sequence is not Delta-k constant'

>dk_constant([4,4,4,4,4,4])
'This sequence is Delta-0 constant'
```
**Note:** As mentioned above, for simplicity we are forcing the list being input to `dk_constant` to have length 6. This is long enough to determine $\Delta^k$-constancy for $k$ up to 5. **The function should give an error if the list is not length 6.**

Technically a sequence that is $\Delta^k$-constant for $k=7$ or higher would be labeled as "not $\Delta^k$-constant" by this function. That's not exactly right but we'll go with it for now, again to keep things simple. **However, your function should detect $\Delta^k$-constancy for $k$ up to 6.** 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwMjYwMjM5MDUsLTE3NDUwMDQ0MzgsND
