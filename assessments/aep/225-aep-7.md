...
```
**Note:** As mentioned above, for simplicity we are forcing the list being input to `dk_constant` to have length 6. This is long enough to determine $\Delta^k$-constancy for $k$ up to 5. Technically a sequence that is $\Delta^k$-constant for $k=7$ or higher would be labeled as "not $\Delta^k$-const
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTExNDAwNjg0NTQsLTE3NDUwMDQ0MzgsND
