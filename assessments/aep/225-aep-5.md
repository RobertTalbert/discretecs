...
...and so on. The value of $3x \ \% \ 6$ will just alternate between 0 and 3, because $3x$ is always either a multiple of $6$ (which happens when $x$ is even) or it's 3 plus a multiple of 6 (which happens when $x$ is odd). So $3x \ \% \ 6 = 1$ *has no solution*; and we would say $3^{-1} \ \% \ 6$ *does not exist*. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNTc1MjYxODU4XX0=
-->