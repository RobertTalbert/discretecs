...
2. Look at your data from the first question and any additional data you might generate through additional examples, and look at the values of $a$ and $n$ when $a^{-1} \ \% \ n$ exist
under what conditions will $a^{-1} \ \% \ n$ exist where $a,n$ are integers? Fill in the blank with what you think is a correct and complete list of conditions: 

**If $a,n$ are integers, then $a^{-1} \ \% \ n$ exists  ______.**

The statement you use to fill in the blank should express some kind of relationship between $a$ and $n$. For example *"...whenever $a$ is odd and $n$ is even"* would be the right kind of condition to give. Although that answer is wrong (look at $a=3$ and $n=6$ for example; this satisfies the condition but $3^{-1} \ \% \ 6$ does not exist), it's the right *kind* of answer. Your job is to find the correct condition. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzNTU5MDczNzNdfQ==
-->