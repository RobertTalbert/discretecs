...
4. Now let's think about how we might find $a^{-1} \ \% \ n$ more efficiently than with a brute-force search. 
   ...
   a) Suppose there are (not necessarily positive) integers $x,y$ such that $ax + ny = 1$. Use the fact that $ax + ny = 1$ to argue that $ax \ \% \ n = 1$. (Hint: Start with the equation $ax + ny = 1$ and apply the modulus operator $\% \ n$ to both sides; use the fact that $(ax + ny) \ \% \ n = (ax \ \% \ n) + (ny \ \% \ n)$.) 
   b) 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjQ4OTc4MjgyXX0=
-->