# AEP 5: Solving congruence equations
...
In algebra, we use a $-1$ exponent to denote a reciprocal, for example $3^{-1} = 1/3$. We will use that negative 1 exponent to mean something similar here: In the situation we just described, we would say that $3^{-1} \ \% \ 7 = 5$ since $3$ times $5$ is congr



But notice that for some moduli, this equation may be impossible to solve. For example, if we were given the equation $3x \ \% \ 6 = 1$ instead, then look what happens: 
...
...and so on. The value of $3x \ \% \ 6$ will just alternate between 0 and 3, because $3x$ is always either a multiple of $6$ (which happens when $x$ is even) or it's 3 plus a multiple of 6 (which happens when $x$ is odd). So $3x \ \% \ 6 = 1$ *has no solution*. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTcxMDM0NDM0MF19
-->