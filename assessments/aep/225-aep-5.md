...
But that doesn't make sense because $1/3$ isn't an integer. Instead, we need to find an *integer* $x$ that, when substituted in, yields a true statement. When the modulus is small (like here, where it's 7) we can do this through a brute-force search pretty easily: 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTEwODA5ODE0OV19
-->