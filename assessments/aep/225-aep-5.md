...
4. Let $a = 92$ and $n = 777$. Use the Euclidean Algorithm to show that $\gcd(a,n) = 1$, and then 


4. Suppose there are (not necessarily positive) integers $x,y$ such that $ax + ny = 1$. Use the fact that $ax + ny = 1$ to argue that $ax \ \% \ n = 1$. (Hint: Start with the equation $ax + ny = 1$ and apply the modulus operator $\% \ n$ to both sides; use the fact that $(ax + ny) \ \% \ n = (ax \ \% \ n) + (ny \ \% \ n)$.) 
   b) Let $a = 92$ and $n = 777$. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNzM4NDY2OTY0XX0=
-->