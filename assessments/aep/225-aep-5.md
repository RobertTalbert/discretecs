...
5. 
Show (similarly to in the first problem) that the $x$ you 


6. Suppose there are (not necessarily positive) integers $x,y$ such that $ax + ny = 1$. Use the fact that $ax + ny = 1$ to argue that $ax \ \% \ n = 1$. (Hint: Start with the equation $ax + ny = 1$ and apply the modulus operator $\% \ n$ to both sides; use the fact that $(ax + ny) \ \% \ n = (ax \ \% \ n) + (ny \ \% \ n)$.) 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTY1MDUyMzM2MF19
-->