...
4. Now let's think about how we might find $a^{-1} \ \% \ n$ more efficiently than with a brute-force search. 

   a) Suppose there are (not necessarily positive) integers $x,y$ such that $ax + ny = 1$. (For example, if $a = 3$ and $n = 7$, then $x = -2$ and $y = 1$ will work: $3(-2) + 7(1) = 1$.) Use the fact that $ax + ny = 1$ to argue that $ax \ \% \ n = 1$. (Hint: Start with the equation $ax + ny = 1$ and apply the modulus operator (
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE4NzE2OTYxMDRdfQ==
-->