...
4. Now let's think about how we might find $a^{-1} \ \% \ n$ more efficiently than with a brute-force search. 
   ...
   a) Suppose there are (not necessarily positive) integers $x,y$ such that $ax + ny = 1$. (For example, if $a = 3$ and $n = 7$, then $x = -2$ and $y = 1$ will work: $3(-2) + 7(1) = 1$.) 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMzIzODg4MDhdfQ==
-->