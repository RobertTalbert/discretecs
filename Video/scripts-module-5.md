...
For example, look at this recurrence relation for a sequence called a_n. It's defined recursively with a recurrence relation a_n = 3a_{n-1} - 2a_{n-2} with initial conditions a_0 = 2 and a_1 = 5. With that information, you can check (and should check if you want practice with working with recursive definitions) that the first five terms of the sequence are 2, 5, 11, 23, 47, 95. If there were a solution to this recurrence relation it would need to produce *every single one* of the *infinitely many* terms of the sequence correctly. It's definitely not obvious what a closed formula for this sequence might be, or even if there is one. But it's very easy to find some examples of formulas that are *not* solutions. A function would *fail* to be a solution if *not all* of terms are output in the same way that the recurrence relation would do it. 

For exmaple, the function f(n) = 3^n is not a solution to this recurrence relation because if we start the inputs at 0, like the recurrence relation does, it's obvious that the function and the sequence do not agree. g(n) = 2^n is also not a solution. What about the function h(n) = 1.5n^2 + 1.5n + 2? Notice that this produces the correct first three terms of the sequence but then fails at the fourth one. 

This illustrates a critically important fact: If a function fails to produce the correct term in a sequence given by a recurrence relation, the function is not a solution. BUT: Just because a function might produce the correct initial conditions and first few terms, it DOES NOT mean that the function is a solution. 

Here for example is a function [r\left(n\right)\ =\ 0.025n^{5}-0.125n^{4}+0.625n^{3}+0.125n^{2}+2.35n+2] that produces all six of the correct terms we started with. Does this make it a solution to the recurrence relation? No, because we haven't seen *all possible outputs*. There may be some output we haven't generated yet from either the recurrence relation *or* the formula where the two disagree. And in fact that's exactly the case: for n = 7, f(7) is this number [359], but a_7 is this number [191]. **Generating examples will never prove that a function solves a recurrence relation.** But **just one counterexample will prove that a function doesn't solve a recurrence relation.**

So if I have a function and I really think it *is* a solution to a recurrence relation, I can't generate a list of outputs because no list of outputs will ever be long enough to conclusively prove anything. Well, then, what would a convincing, conclusive, general explanation look like? 

We need to change our perspective here away from checking specific outputs of the function we have, because those can't be used to prove a function is a solution to a recurrence relation. Instead, let's think about what this function really should *do* to be a solution. It must do two things: First the function has to produce the right initial conditions. If we plug in the same value as the index in the beginning, the outputs must equal the initial conditions. Then, the function's outputs have to obey the same rules of recursion as the recurrence relation. So if I replace all terms of the sequence in the recurrence relation with their corresponding function values --- a_n replaced with f(n), a_{n-1} replaced with f(n-1), and so on --- the resulting equation has to be true. 

Let's look at an example of how this works with a very simple recurrence relation. This comes from the sequence 5, 8, 11, 14, 17, and so on that we saw in an earlier video. This is an arithmetic sequence and its recursive definition would look like this. If we start the indexing at 0, then a_0 = 5 and for n > 0, a_n = a_{n-1} + 3. First of all, is f(n) = 2n + 5 a solution? First of all check the initial condition: If n = 0, then f(0) = 2(0) + 5 = 5, so the initial condition does work. But then from there the outputs of the function are all wrong, so in fact this is not a solution. 

What about f(n) = 3n + 5? Well, the initial condition is still satisfied; and the terms of the sequence look right, but remember this doesn't mean anything. So instead, look at the recurrence relation. Separate the right side from the left. On the left, replace a_n with its corresponding function value f(n) which is 3n + 5. On the right, do the same. f(n-1) is 3(n-1) + 5, and we are adding 3 onto this in the recurrence relation. Now do the math: 3(n-1) + 5 + 3 is 3n -3 + 5 - 3, which is 3n+5. Now look at the two sides. The left side equals the right side. Because the function f(n) = 3n + 5 satisfies the recurrence relation and has the correct initial condition, that makes it a solution to the recurrence relation. 

This is an airtight general explanation because we didn't use specific examples except when checking to make sure the initial condition was met. Otherwise we used the general formula with no specific inputs and showed that this formula, at a generic input we just called "n", satisfies the recurrence relation. Since no specifics were used, this recurrence relation is satisfied by the function everywhere. 

Let's look at another example, going back to the earlier recurrence relation a_n = 3a_{n-1} - 2a_{n-2} with initial conditions a_0 = 2 and a_1 = 5. I claim that this function, f(n) = 3*2^n - 1 is a solution. I'm not telling how I actually got this function --- that's coming in a later video --- I just want to prove that it solves the recurrence relation. So let's first check to make sure the initial conditions are satisfied. When n = 0 the sequence term is supposed to be 2. And if I plug in n = 0 to the formula, I get 3 times 2^0 - 1 which is 3 times 1 - 1 which is indeed 2. Likewise n = 1 should produce 5, and if I plug in n = 1 I get 3 times 2^1 - 1 which is actually 5. So the initial conditions are satisfied. 

That's all the specific numbers we are going to plug in. Instead, we're now going to substitute the formula in for the sequence terms in the recurrence relation and do the math to see if the left and right sides equal each other. Separate the left and right sides. On the left we would replace a_n with the appropriate function value which is 3 times 2^n - 1. On the right, I would do the same, replacing a_{n-1} with f(n-1) and a_{n-2} with f(n-2). Then use the formula to get: 3 times the quantity 3 times 2^(n-1) - 1, plus 2 times the quantity 3 times 2^(n-2) - 1. I'm now going to simplify all this. This involves algebra, which I'll do without explanation but if you see a step here that is unfamiliar to you, pause the video and try to work it out, and go search the internet or ask a friend to fill in the gaps. 
