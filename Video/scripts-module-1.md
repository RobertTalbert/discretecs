...
Subtraction in binary also involves borrowing. Let's see how this works with the example 1101 - 110. In decimal this would be 13 minus 6, so the answer should be 7. 

Lining these up with the larger one on top, in the ones place we have 1 - 0. this is just 1, so put that down with no extra steps needed. In the 2's place we have something to think about. 0 - 1 doesn't compute. It's analogous to having 2 - 5 in the earlier base 10 example. It doesn't make sense to put a negative number in this place in the result. So here we borrow from the next place up. But it's a little tricky. Borrowing reduces the 1 that's in the 8's place to a 0. And although it says "1", it's from the next place up --- so it's actually a 2, which in binary we write as 10. Again: We are borrowing 1 copy of the higher place value, which in this case is 8, and bringing it to the lower place value, which is 4. That 1 is twice the size of the digits in the current place.  

So in our example, we have 10 minus 1, which is really 2 - 1 so the answer is just 1. 
