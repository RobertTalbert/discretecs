...
In Module 4B we introduce the idea of a **predicate**, which is a logical proposition that involves variables. A predicate is essentially a function that returns `TRUE` or `FALSE` depending on what the input is. Some predicates return `TRUE` for only *some* values of input, while others *always* return `TRUE` or maybe *never* return `TRUE`. Determining when a predicat
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTg0NDEzNDkyNSw2ODU0NDI5MDRdfQ==
-->